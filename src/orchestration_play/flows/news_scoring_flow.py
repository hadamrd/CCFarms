...
def initialize_digger(
    ...
    digger.set_news_client(news_block.get_client())
    ...
    digger.set_article_cache(cache_block.get_article_cache())
...
def send_notification(articles: List[Dict], query: str, webhook_name: str = "weather-teams-webhook"):
    ...
    try:
        ...
        if not articles:
            ...
            message += "\n".join([f"{idx}. {article['title']} - Score: {article['score']}" 
                                 for idx, article in enumerate(articles, 1)])
...
def news_scoring_flow(
    ...
    send_notification(
        ...
        query=query, 
        webhook_name=teams_webhook_name
    )
